...
const HabitManager = () => {
  ...
  const [newHabit, setNewHabit] = useState({ title: '', nature: 'Good' });
      ...
      <div className="habit-list space-y-4">
        {habits.length > 0 && habits.map(habit => (
          ...
          >
            ...
            <p className="text-sm mb-2">Nature: {habit.nature}</p>
            <p className="text-sm mb-2">Streak: {habit.streak}</p>
            <div className="habit-actions flex space-x-2">
              <button 
                onClick={() => updateHabit(habit.id, habit.nature ? 'Good': 'Bad')}
